...
class TSP(object):
    ...
    def init(self):
        tsp = self
        data_file = input("请输入文件名称: ")
        tsp.load_Citys(data_file)  # 加载城市数据
        tsp.pop = tsp.creat_pop(tsp.pop_size)  # 创建种群
        tsp.fitness = tsp.get_fitness(tsp.pop)  # 计算初始种群适应度
        tsp.dw.bound_x = [np.min(tsp.citys[:, 0]), np.max(
            tsp.citys[:, 0])]  # 计算绘图时的X界
        tsp.dw.bound_y = [np.min(tsp.citys[:, 1]), np.max(
            tsp.citys[:, 1])]  # 计算绘图时的Y界
        tsp.dw.set_xybound(tsp.dw.bound_x, tsp.dw.bound_y)  # 设置边界

    def creat_pop(self, size):
        pop = []
        for i in range(size):
            # 问题的解，基因，种群中的个体：[0，...，city_size]
    ...
    def get_local_fitness(self, gen, i):
        ...
        for j in range(self.city_size):
            if i != j:
                od.append(self.ct_distance(
                    self.citys[gen[i]], self.citys[gen[i - 1]]))
        mind = np.min(od)
        fi = di - mind
        return fi
    ...
    def select_pop2(self, pop):
        ...
        idx = np.random.choice(np.arange(self.pop_size),
                               size=self.pop_size, replace=True, p=probility)
        n_pop = pop[idx, :]
        return n_pop
    ...
    def evolution(self):
        ...
        for i in range(self.ga_num):
            ...
            print('迭代次数:%d 次 , 最短路径长度 :%s' % (i+1, self.best_dist))
            ...
            for j in range(self.pop_size):
                ...
                if j != r:
                    tsp.pop[j] = tsp.cross(
                        tsp.pop[j], tsp.pop[r])  # 交叉种群中第j,r个体的基因
    ...
    def gen_distance(self, gen):
        ...
        for i in range(-1, len(self.citys) - 1):
            ...
            distance += np.sqrt((city1[0] - city2[0])
                                ** 2 + (city1[1] - city2[1]) ** 2)
        return distance
    ...
    def draw_citys_name(self, gen, size=5):
        ...
        for i in range(m):
            ...
            tsp.dw.draw_text(
                best_icity[0], best_icity[1], tsp.citys_name[c], 10)
...
def main():
    ...
    tsp.dw.plt.show()
    # 输出最终结果
    print(f"迭代{tsp.ga_num}次之后，最终的最短路径如图，最短路径的长度是{tsp.best_dist}")
